...
module.exports = {
  getUsersByOffsetBased: async (offset, limit) => {
    ...
    const data = await conn
      ...
      .query("SELECT * FROM Users ORDER BY userID OFFSET @offset ROWS FETCH NEXT @limit ROWS ONLY");
  ...
  getUsersByKeysetBased: async (lastID, limit) => {
    ...
    const data = await conn
      ...
      .input("limit", sql.Int, limit)
      .query("SELECT TOP (@limit) * FROM Users WHERE userID > @lastID ORDER BY userID");
  ...
  createUser: async (user) => {
    ...
    const data = await conn
      ...
      .query("INSERT INTO Users (username, email, password, role, createAt) OUTPUT inserted.userID, inserted.role VALUES (@username, @email, @password, @role, @createAt)");
  ...
  updateUser: async (user) => {
    ...
    await conn
      ...
      .query("UPDATE Users SET username = @username, email = @email, password = @password, role = @role, updateAt = @updateAt WHERE userID = @userID");
  },
