...
module.exports = {
  login: async (req, res, next) => {
    ...
    if (user.length === 0 || user[0].password !== password) {
      return Response.error(res, Message.ERROR_INVALID_CREDENTIALS, null, StatusCode.UNAUTHORIZED);
    }
  ...
  register: async (req, res, next) => {
    ...
    return Response.success(res, Message.SUCCESS_REGISTER, newData, StatusCode.CREATED);
  },

  check: async (req, res, next) => {
    if (!req.session.userID) {
      return Response.error(res, Message.ERROR_UNAUTHORIZED, null, StatusCode.UNAUTHORIZED);
    }
    return Response.success(res, Message.SUCCESS_LOGIN, {
